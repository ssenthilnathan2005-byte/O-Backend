...
const MIGRATIONS = [
  ...
  "ALTER TABLE doctors ADD COLUMN IF NOT EXISTS schedule_config TEXT",
  ...
  "ALTER TABLE prescriptions ADD COLUMN IF NOT EXISTS patient_confirmed INTEGER NOT NULL DEFAULT 0",
  ...
  `CREATE TABLE IF NOT EXISTS lab_test_offerings (
    id          TEXT PRIMARY KEY,
  ...
  `CREATE TABLE IF NOT EXISTS lab_bookings (
    ...
    collection_type   TEXT NOT NULL DEFAULT 'home' CHECK(collection_type IN ('home','walk_in')),
    address           TEXT,
    price             REAL NOT NULL,
    payment_done      BOOLEAN NOT NULL DEFAULT FALSE,
    status            TEXT NOT NULL DEFAULT 'booked' CHECK(status IN ('booked','technician_assigned','sample_collected','processing','report_ready','cancelled')),
    report_url        TEXT,
    notes             TEXT,
    created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at        TIMESTAMPTZ NOT NULL DEFAULT now()
  ...
  "CREATE INDEX IF NOT EXISTS idx_lab_test_sessions_lab ON lab_test_sessions(lab_id)",
  "UPDATE lab_bookings lb SET token_number = sub.rn FROM (SELECT id, ROW_NUMBER() OVER (PARTITION BY lab_id, test_id, slot_date ORDER BY created_at, id) AS rn FROM lab_bookings WHERE token_number IS NULL) sub WHERE lb.id = sub.id AND lb.token_number IS NULL",
  "UPDATE lab_bookings SET slot_time='morning' WHERE slot_time NOT IN ('morning','afternoon')",
  "UPDATE lab_test_sessions SET session_id = session_id || '_morning' WHERE session_id !~ '_(morning|afternoon)$'",
  "DROP INDEX IF EXISTS uq_lab_bookings_token",
  "CREATE UNIQUE INDEX IF NOT EXISTS uq_lab_bookings_token_session ON lab_bookings(lab_id, test_id, slot_date, slot_time, token_number) WHERE token_number IS NOT NULL",
  "ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check",
  "ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role = ANY (ARRAY['patient'::text, 'doctor'::text, 'admin'::text, 'hospital_admin'::text, 'lab_admin'::text]))",
  "ALTER TABLE labs ADD COLUMN IF NOT EXISTS login_id TEXT UNIQUE",
  "ALTER TABLE labs ADD COLUMN IF NOT EXISTS admin_user_id TEXT REFERENCES users(id) ON DELETE SET NULL",
  "ALTER TABLE labs ADD COLUMN IF NOT EXISTS home_collection BOOLEAN NOT NULL DEFAULT FALSE",
  "ALTER TABLE labs ADD COLUMN IF NOT EXISTS home_collection_fee REAL NOT NULL DEFAULT 0",
  "ALTER TABLE labs ADD COLUMN IF NOT EXISTS map_location TEXT",
  "ALTER TABLE doctors ADD COLUMN IF NOT EXISTS doctor_fee REAL",
];
